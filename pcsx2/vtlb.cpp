...
struct VTLBFastmemCompactLogState
{
	bool active = false;
...
struct VTLBUnmapCompactLogState
{
	bool active = false;
	u32 start_vaddr = 0;
	u32 last_vaddr = 0;
...
static VTLBUnmapCompactLogState s_vtlb_unmap_log_state;
...
{
	if (!s_vtlb_fastmem_log_state.active)
		return;

	const VTLBFastmemCompactLogState& state = s_vtlb_fastmem_log_state;
	vtlb_LogEventCompact("fastmem_map", fmt::format("{} count={} vaddr={:08X}-{:08X} mainmem={:08X}-{:08X} prot={}",
		state.is_create ? "create" : "remove", state.count,
		state.start_vaddr, state.last_vaddr + VTLB_PAGE_SIZE,
		state.start_mainmem, state.last_mainmem + VTLB_PAGE_SIZE,
		state.prot));

	s_vtlb_fastmem_log_state = {};
}

static void vtlb_LogFastmemCompact(bool is_create, u32 vaddr, u32 mainmem_offset, const char* prot)
{
	VTLBFastmemCompactLogState& state = s_vtlb_fastmem_log_state;
	if (!state.active)
	{
		state.active = true;
		...
		state.start_mainmem = mainmem_offset;
...
static void vtlb_FlushVMapCompactLog()
{
	if (!s_vtlb_map_log_state.active)
		return;

	const VTLBMapCompactLogState& state = s_vtlb_map_log_state;
	vtlb_LogEventCompact("vmap", fmt::format("batch op=map count={} vaddr={:08X}-{:08X} paddr={:08X}-{:08X} size_each={:X} total={:X}",
...
static void vtlb_FlushVMapUnmapCompactLog()
{
	if (!s_vtlb_unmap_log_state.active)
		return;

	const VTLBUnmapCompactLogState& state = s_vtlb_unmap_log_state;
	vtlb_LogEventCompact("vmap_unmap", fmt::format("batch op=unmap count={} vaddr={:08X}-{:08X} size_each={:X} total={:X}",
		state.count, state.start_vaddr, state.last_vaddr + state.size_each, state.size_each, state.count * state.size_each));

	s_vtlb_unmap_log_state = {};
}

static void vtlb_LogVMapCompact(u32 vaddr, u32 paddr, u32 size)
{
	vtlb_FlushVMapUnmapCompactLog();
...
{
	vtlb_FlushVMapCompactLog();
	VTLBUnmapCompactLogState& state = s_vtlb_unmap_log_state;
	if (!state.active)
	{
		state.active = true;
		state.start_vaddr = vaddr;
	...
	const bool contiguous = (state.size_each == size && state.last_vaddr + state.size_each == vaddr);
	if (!contiguous)
	{
		vtlb_FlushVMapUnmapCompactLog();
		state.active = true;
...
static void vtlb_CreateFastmemMapping(u32 vaddr, u32 mainmem_offset, const PageProtectionMode& mode)
{
	...
	vtlb_LogFastmemCompact(true, vaddr, mainmem_offset, vtlb_PageProtectionToString(mode));
...
static void vtlb_RemoveFastmemMapping(u32 vaddr)
{
	...
	vtlb_LogFastmemCompact(false, vaddr, mainmem_offset, "n/a");
